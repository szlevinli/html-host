...
async def test_upload_requires_auth(client: AsyncClient) -> None:
    r = await client.post("/v1/files", files={"file": ("test.html", b"<h1>hi</h1>", "text/html")})
    assert r.status_code == 401


@pytest.mark.asyncio
async def test_upload_and_list(client: AsyncClient, auth_headers: dict[str, str]) -> None:
    ...
    assert data["url"].endswith(data["short_code"])
